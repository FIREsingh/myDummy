...
import useGetExperienceById from '@/hooks/useGetExperienceById';

const UpdateExperienceForm = () => {
    const params = useParams();
    const id = params.id;
    useGetExperienceById(id);
    const { singleExperience } = useSelector(store => store.experience)
    ...
    const submitHandler = async (e) => {
        ...
        try {
        const res = await axios.post(`${EXPERIENCE_API_END_POINT}/${id}/edit`, formData, {
            headers: {
    ...
    useEffect(() => {
        ...
        setInput({
            position: singleExperience.position || "",
            company: singleExperience.company || "",
            fromDate: extractDate(singleExperience.from) || undefined,
            toDate: extractDate(singleExperience.to) || undefined,
        ...
        <div>
            ...
            <div className="flex items-center justify-center max-w-7xl mx-auto">
                <form onSubmit={submitHandler} className="w-1/2 border border-gray-200 rounded-md p-4 my-10" encType='application/json'>
                    <h1 className="text-xl font-titleFonts font-bold mb-5">Update Experience</h1>
